Page({
  ...
  onShow: function() {
    this.loadCollectedDynamics()
  },

  loadCollectedDynamics: function() {
    const collectedIds = wx.getStorageSync('collectedDynamics') || []
    const allDynamics = this.getAllDynamics()
    const collectedDynamics = allDynamics.filter(item => collectedIds.includes(item.id))
    this.setData({ list: collectedDynamics })
  },
